...
const Product_Card = ({ productName, description }) => {
  ...
  return (
    <div className="max-w-[420px] bg-[#322E2E] border  rounded-3xl shadow-xl shadow-[#6D98EC]/40" style={{ background: 'linear-gradient(to bottom, #0A0A0A 100%, #302E2E 24%)' }}>
      <a href="#">
        <Image className="rounded-t-3xl" src="/product_card_1.png" width={420} height={300} alt='Growth OS' />
      </a>
      <div className="p-5">
        <div className='my-3'>
          {productName !== "GROWTH OS" ? <h2 className='text-[#FFFBEF] font-bold text-2xl'>{firstWord} <br /> <span className='text-[#3877F0]'>{secondWord}</span> </h2> :
