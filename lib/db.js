...
/**
 * Fetches all communities from the 'communities' node
 */
export async function getAllCommunities() {
  try {
    ...
    if (snapshot.exists()) {
      const data = snapshot.val();
      const commArray = Object.keys(data).map(key => ({
        id: key,
        ...data[key]
      }));
      return commArray;
    ...
    // Silently catch permission denied errors to allow mock data fallback
    return [];
  }
}

/**
 * Fetches all mentors from the 'mentors' node
 */
